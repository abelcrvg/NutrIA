...
class _LoginPageState extends State<LoginPage> {
  ...
  Future<void> _submit() async {
    ...
    if (_registering && _name.text.trim().isEmpty) {
      _message('Informe seu nome para criar a conta.');
      return;
    }
  ...
  Future<void> _resetPassword() async {
    ...
    if (email.isEmpty || !email.contains('@')) {
      _message('Informe seu e-mail primeiro.');
      return;
    }
    try {
                ...
                children: [
                  Center(
                    child: Container(
                      ...
                      child: const Icon(Icons.spa_outlined, size: 42, color: NutriTheme.green),
                    ),
                  ),
                  ...
                  Text(
                    _registering ? 'Comece seu diário alimentar.' : 'Entenda melhor o que você come.',
                    textAlign: TextAlign.center,
                      ...
                      children: [
                        ...
                        FilledButton.icon(
                          ...
                          icon: _loading
                              ...
                              : Icon(_registering ? Icons.person_add_alt_1 : Icons.login),
                          label: Text(
                            _loading ? 'Aguarde...' : (_registering ? 'Criar minha conta' : 'Entrar'),
                          ),
                        ),
                        const SizedBox(height: 10),
                        TextButton(
                          onPressed: _loading ? null : () => setState(() => _registering = !_registering),
                          child: Text(_registering ? 'Já tenho uma conta' : 'Ainda não tenho conta'),
                        ),
                      ],
                    ),
                  ),
                  ...
                  const Row(
                    ...
                    children: [
                      ...
                      SizedBox(width: 6),
                      Text('Seus dados ficam vinculados à sua conta', style: TextStyle(fontSize: 12)),
                    ],
                  ),
                ],
              ),
