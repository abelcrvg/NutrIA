...
final Map<String, MealFeedback> _feedbackByCanonical = {for(final entry in mealFeedbackExtras.entries)_canonical(entry.key):entry.value};
...
MealItemAnalysis analyzeFoodItem(String food,double qty,String unit){
  ...
  final foodName=humanNames[key]??(food.isEmpty?'Alimento':food[0].toUpperCase()+food.substring(1));
  ...
  return MealItemAnalysis(name:type=='processed'?'Ultraprocessado':type=='protein'?'Proteína':type=='carb'?'Carboidrato':type=='fiber'?'Fibra':'Alimento',foodName:foodName,type:type,isWarning:processed.containsKey(key),detail:detail,caloriesPer100g:calPer100,unitWeights:weights,actualCalories:calPer100*factor,actualProtein:((dbInfo?['protein'] as num?)??0).toDouble()*factor,actualCarbs:((dbInfo?['carbs'] as num?)??0).toDouble()*factor,actualFat:((dbInfo?['fat'] as num?)??0).toDouble()*factor,actualFiber:((dbInfo?['fiber'] as num?)??0).toDouble()*factor);
}

double portionStep(String unit){if(unit=='grama'||unit=='g')return 10;if(unit=='unidade'||unit=='ovo'||unit=='fatia')return 1;return 1;}
...
MealAnalysisReport findMealAnalysisSmart(String input,num? calories){
  final normalized=normalizeMealText(input);final rawParts=normalized.split(',').map((s)=>s.trim()).where((s)=>s.isNotEmpty).toList();const commonUnits={'colher','concha','pacote','unidade','fatia','porcao','grama','g','xicara','pedaco','ovo','bife','file','pote'};final items=<MealItemAnalysis>[];
  for(final part in rawParts){final words=part.split(RegExp(r'\s+'));final numIdx=words.indexWhere((w)=>double.tryParse(w)!=null);final qty=numIdx==-1?1.0:double.parse(words[numIdx]);var unit='unidade';if(numIdx!=-1&&numIdx+1<words.length&&commonUnits.contains(words[numIdx+1]))unit=words[numIdx+1];final foods=words.where((w)=>double.tryParse(w)==null&&!commonUnits.contains(w)).toList();if(foods.length==1)items.add(analyzeFoodItem(foods.first,qty,unit));else for(final food in foods)items.add(analyzeFoodItem(food,1,'unidade'));}
  if(rawParts.length==1&&items.length<=1){final tokens=normalized.split(' ').where((w)=>w.isNotEmpty&&!commonUnits.contains(w)&&double.tryParse(w)==null).toList();if(tokens.length>1){items.clear();for(final food in tokens)items.add(analyzeFoodItem(food,1,'unidade'));}}
