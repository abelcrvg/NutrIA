      ...
      child: ListView(
        ...
        children: [
          ...
          NutrIACard(
            child: Column(
              children: [
                Row(
                  children: [
                    const NutrIABadge(text: 'Hoje', icon: Icons.today_outlined),
                    const Spacer(),
                    Flexible(
                      child: Text('Meta 2.100 kcal', textAlign: TextAlign.end, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 12)),
                    ),
                  ],
                ),
                const SizedBox(height: 20),
                        ...
                        children: [
                          ...
                          Padding(
                            ...
                            child: Column(
                              ...
                              children: [
                                Text(
                                  _hasTodayCalories ? _todayCalories.toStringAsFixed(0) : '—',
                                  style: Theme.of(context).textTheme.headlineMedium?.copyWith(
                                        fontWeight: FontWeight.w900,
                                        color: NutriTheme.greenDark,
                                      ),
                                ),
                                const SizedBox(height: 4),
                                Text(
                                  _hasTodayCalories ? 'kcal' : 'pendentes',
                ...
                Row(
                  children: [
                    _StatValue(label: 'Refeições', value: meals.length.toString()),
                    const _StatValue(label: 'Meta', value: '2.100'),
                    _StatValue(label: 'Água', value: '$_water ml'),
                  ],
                ),
              ],
            ),
          ...
          Row(
            children: [
              Expanded(
                child: Text('Refeições de hoje', style: Theme.of(context).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.w800)),
              ),
              if (meals.isNotEmpty)
                TextButton(
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      const Text('Hidratação', style: TextStyle(fontWeight: FontWeight.w800)),
                      const SizedBox(height: 3),
                      Text('$_water ml registrados hoje', maxLines: 1, overflow: TextOverflow.ellipsis),
                    ],
                  ),
                ),
                const SizedBox(width: 8),
                OutlinedButton(
                  onPressed: () => setState(() => _water += 250),
        ...
        children: [
          ...
          NutrIACard(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const NutrIABadge(text: 'Leitura do período', icon: Icons.insights_outlined),
                const SizedBox(height: 16),
                Text('${_meals.length} refeições registradas', style: Theme.of(context).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.w800)),
                const SizedBox(height: 6),
                const Text('As calorias só aparecem quando houver um valor nutricional calculado. O NutrIA não inventa números para preencher o histórico.'),
              ],
            ),
          ),
        ],
            ...
            child: Column(
              children: [
                CircleAvatar(
                  radius: 34,
                  backgroundColor: NutriTheme.mint,
                  child: Text(_initial, style: const TextStyle(fontSize: 25, color: NutriTheme.green, fontWeight: FontWeight.w800)),
                ),
...
class _MealTile extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => NutrIACard(
        ...
        child: Row(
          children: [
            Container(
              ...
              child: const Icon(Icons.restaurant_outlined, color: NutriTheme.green),
            ),
            const SizedBox(width: 12),
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(meal.type.toUpperCase(), maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 10, fontWeight: FontWeight.w700)),
                  const SizedBox(height: 3),
                  Text(meal.name, maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(fontWeight: FontWeight.w700)),
                ],
              ),
            ),
            const SizedBox(width: 8),
            SizedBox(
              ...
              child: FittedBox(
                ...
                child: Text(meal.calories == null ? '—' : '${meal.calories!.toStringAsFixed(0)} kcal', maxLines: 1, style: const TextStyle(fontWeight: FontWeight.w800)),
              ),
            ),
          ],
        ),
        ...
        child: Column(
          children: [
            Container(
              width: 54,
              height: 54,
              decoration: BoxDecoration(color: NutriTheme.mint, borderRadius: BorderRadius.circular(17)),
              child: const Icon(Icons.restaurant_outlined, color: NutriTheme.green, size: 28),
            ),
            const SizedBox(height: 14),
            Text('Nenhuma refeição registrada', textAlign: TextAlign.center, style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.w800)),
            const SizedBox(height: 6),
            const Text('Quando você adicionar uma refeição, ela aparecerá aqui. Nada é criado automaticamente.', textAlign: TextAlign.center),
            const SizedBox(height: 16),
            OutlinedButton.icon(
              onPressed: onPressed,
              icon: const Icon(Icons.add),
              label: const Text('Registrar primeira refeição'),
            ),
          ],
        ),
      );
