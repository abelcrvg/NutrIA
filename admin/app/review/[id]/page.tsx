...
export default function ReviewPage(){
 const params=useParams<{id:string}>(); const id=params.id; const [form,setForm]=useState<Form>({}); const [submission,setSubmission]=useState<any>(null); const [photoUrl,setPhotoUrl]=useState(''); const [photoLoading,setPhotoLoading]=useState(false); const [loading,setLoading]=useState(true); const [saving,setSaving]=useState(false); const [message,setMessage]=useState(''); const [error,setError]=useState('');
 useEffect(()=>{(async()=>{const {data:{session}}=await supabase.auth.getSession();if(!session){window.location.href='/';return;}const {data:profile}=await supabase.from('profiles').select('is_admin').eq('id',session.user.id).maybeSingle();if(!profile?.is_admin){await supabase.auth.signOut();window.location.href='/';return;}const {data,error}=await supabase.from('product_submissions').select('*').eq('id',id).single();if(error||!data){setError(error?.message??'Submissão não encontrada.');setLoading(false);return;}setSubmission(data);const extracted=(data.extracted_data??{}) as Record<string,unknown>;const initial:Form={};for(const [key] of fields)initial[key]=extracted[key] == null ? '' : String(extracted[key]);setForm(initial);setLoading(false);
   if(data.photo_path){
     setPhotoLoading(true);
     const {data:signed,error:signedError}=await supabase.storage.from('product-labels').createSignedUrl(data.photo_path,600);
     if(signed?.signedUrl)setPhotoUrl(signed.signedUrl);
     else setError(signedError?.message??'Não foi possível carregar a foto da tabela nutricional.');
 ...
 <div className="reviewgrid"><section className="card"><h2 className="section-title">Evidência enviada</h2><div className="photo">{photoLoading?<p>Carregando foto da tabela nutricional…</p>:photoUrl?<img src={photoUrl} alt="Tabela nutricional enviada" style={{maxWidth:'100%',maxHeight:600,objectFit:'contain'}}/>:<div><strong>Foto da tabela nutricional</strong><p>{submission.photo_path||'Nenhuma foto disponível.'}</p>{submission.photo_path&&<small>Não foi possível obter uma URL temporária para esta evidência.</small>}</div>}</div></section>
